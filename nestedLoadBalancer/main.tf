...
resource "oci_load_balancer_listener" "publicLB_http_listener" {
    ...
    protocol = "HTTP"
    //routing_policy_name = oci_load_balancer_load_balancer_routing_policy.pubLB_main_routing_policy.name
    rule_set_names = [oci_load_balancer_rule_set.pubLB_rule_set.name]
}

resource "oci_load_balancer_rule_set" "pubLB_rule_set" {
    #Required
    items {
        #Required
        action = "REDIRECT"
